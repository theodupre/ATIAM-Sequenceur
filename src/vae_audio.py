...
class VAE_AUDIO(nn.Module):
    def __init__(self, input_size, convParams, encMlpParams, D_z, deconvParams, decMlpParams):
        ...
        self.decMlp1 = nn.Linear(decMlpParams[0][0],decMlpParams[0][1])
        ...
        self.outMu = nn.Linear(decMlpParams[1][1], 28*28)
        self.outVar = nn.Linear(decMlpParams[1][1], 28*28)
    ...
    def encoder(self,x):
        ...
        h_conv = self.conv(x)
        h_conv_flat = torch.flatten(h_conv)
    ...
    def decoder(self, z_sample):
        ...
        h_mlp = F.relu(self.decMlp2(F.relu(self.decMlp1(z_sample))))
